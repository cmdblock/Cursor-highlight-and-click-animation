...
{
	g_hWnd = hMainWnd;
	g_hMouseHook = SetWindowsHookEx(WH_MOUSE_LL, MouseProc, g_hInstance, 0);
...
LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == WM_MOUSEMOVE)
	{
		...
		if (bFirstMove)
		{
			...
			GetCursorPos(&ptFrontCursor);
			bFirstMove = FALSE;
		}
		else
		{
			GetCursorPos(&ptCurrentCursor);
			if (sqrt(pow(ptCurrentCursor.x - ptFrontCursor.x, 2) + pow(ptCurrentCursor.y - ptFrontCursor.y, 2)) > 5)
			{
				GetCursorPos(&ptFrontCursor);
				SendMessage(g_hWnd, WM_USER, wParam, lParam);
			}
	...
	{
		//PostMessage(g_hWnd, WM_USER, wParam, lParam);
		SendMessage(g_hWnd, WM_USER + 2, wParam, lParam);
	}
	return CallNextHookEx(g_hMouseHook, nCode, wParam, lParam);
}
